...
export default {
  data() {
    return {
      ...
      regionAry: [], //省份选择框
      defaultName: "", //下拉框的值
      deleteUrl: Interface.DELETE_GOODS,
      addUrl: Interface.QUERY_ALL_GOOD,
      addList: Interface.ADD_LIST_GOOD
    };
  },
  methods: {
    //每页目录条数改变时发生
    ...
    joinData(arr) {
      ...
      this.$http
        ...
        .then(res => {
          ...
          this.$http.post(Interface.QUERYGOOD, sendArr).then(res2 => {
            Object.keys(res2.data.data).forEach((item, index) => {
              tableSource[index].fService = res2.data.data[item].lg;
            });
