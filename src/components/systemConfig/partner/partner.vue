...
export default {
  data() {
    return {
      ...
      regionAry: [], //省份选择框
      defaultName: "", //下拉框的值
      deleteUrl: Interface.AREA_OPEN_CAR_DELETE,
      addUrl: Interface.CAR_TYPE_LIST_SELECT,
      addList: Interface.AREA_OPEN_CAR_ADD
    };
  },
  methods: {
    //每页目录条数改变时发生
    ...
    joinData(arr) {
      ...
      this.$http
        ...
        .then(res => {
          ...
          this.$http
            .post(Interface.AREA_OPEN_CAR_SELECT, sendArr)
            .then(res2 => {
              //console.log(res2.data.data);
              res2.data.data.forEach((item, index) => {
                var attr = Object.keys(item)[0];
                var data = item[attr];
                tableSource[index].fService = data;
              });
