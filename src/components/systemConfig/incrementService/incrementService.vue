      ...
      >
          ...
          <div class="addService-top">
            <handle-table
              ...
              :addList="addList"
              serviceName="附加"
              param1="tIncrementService"
              param2="fServiceName"
              param3="fIncrementServiceId"
...
export default {
  data() {
    return {
      ...
      regionAry: [], //省份选择框
      defaultName: "", //下拉框的值
      deleteUrl: Interface.DELETE_AREA_INCREMENT,
      addUrl: Interface.QUERY_ALL_INC,
      addList: Interface.ADD_LIST_INC
    };
  },
  methods: {
    //每页目录条数改变时发生
    ...
    joinData(arr) {
      ...
      this.$http
        ...
        .then(res => {
          ...
          this.$http.post(Interface.QUERYINC, sendArr).then(res2 => {
            Object.keys(res2.data.data).forEach((item, index) => {
              tableSource[index].fService = res2.data.data[item].li;
            });
